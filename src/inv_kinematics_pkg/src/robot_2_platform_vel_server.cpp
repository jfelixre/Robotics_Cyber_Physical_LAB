...
rclcpp::Node::SharedPtr Robot2PlatformVelServer_node = nullptr;
...
int main(int argc, char **argv)
{
  ...
  Robot2PlatformVelServer_node = rclcpp::Node::make_shared("robot_2_platform_vel_server");


  rclcpp::Service<interfaces::srv::PlatformVel>::SharedPtr service =
    Robot2PlatformVelServer_node->create_service<interfaces::srv::PlatformVel>("robot_2/set_platform_vel", &send_vel);

  publisher_M = Robot2PlatformVelServer_node->create_publisher<interfaces::msg::MotorPlatformVels>("/robot_2/platform_vel_motors",1);

  RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Ready to send velocities to Robot 2 Platform.");

  rclcpp::spin(Robot2PlatformVelServer_node);
  rclcpp::shutdown();
}