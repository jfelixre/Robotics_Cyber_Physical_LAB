...
//ros2 topic pub --once  /robot_platform_1/vel_Motors interfaces/msg/MotorVels "{vel_m1: 20}"
...
#include <gazebo_plugin_sim/platform1_plugin.hpp>
...
namespace gazebo_plugin_sim
{
	...
	Platform1Plugin::Platform1Plugin()
	: impl_(std::make_unique<Platform1PluginPrivate>())
	{
	...
	void Platform1Plugin::Load(gazebo::physics::ModelPtr _model, sdf::ElementPtr _sdf)
	{
		impl_->model_ = _model;
    	...
    	impl_->SetVelocityM4(velocityM4);
      	...
      	impl_->vel_M_ = impl_->ros_node_->create_subscription<interfaces::msg::MotorVels>(
    		...
    		std::bind(&Platform1PluginPrivate::OnMsgVelM, impl_.get(), std::placeholders::_1));
      	...
      	impl_->updateConnection = gazebo::event::Events::ConnectWorldUpdateBegin(
      		std::bind(&Platform1PluginPrivate::OnUpdate, impl_.get(), std::placeholders::_1));

	}

	//Void set velocity
	void Platform1PluginPrivate::SetVelocityM1(const double &_velM1)
	{
		Platform1PluginPrivate::model_->GetJointController()->SetVelocityTarget(
			Platform1PluginPrivate::jointM1->GetScopedName(),_velM1);
		RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "\nSet_Velocity_Platform1_M1= %f", _velM1);
	}

	void Platform1PluginPrivate::SetVelocityM2(const double &_velM2)
	{
		Platform1PluginPrivate::model_->GetJointController()->SetVelocityTarget(
			Platform1PluginPrivate::jointM2->GetScopedName(),_velM2);
		RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "\nSet_Velocity_Platform1_M2= %f", _velM2);
	}

	void Platform1PluginPrivate::SetVelocityM3(const double &_velM3)
	{
		Platform1PluginPrivate::model_->GetJointController()->SetVelocityTarget(
			Platform1PluginPrivate::jointM3->GetScopedName(),_velM3);
		RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "\nSet_Velocity_Platform1_M3= %f", _velM3);
	}

	void Platform1PluginPrivate::SetVelocityM4(const double &_velM4)
	{
		Platform1PluginPrivate::model_->GetJointController()->SetVelocityTarget(
			Platform1PluginPrivate::jointM4->GetScopedName(),_velM4);
		RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "\nSet_Velocity_Platform1_M4= %f", _velM4);
	}


	//Callback subscriptions
	void Platform1PluginPrivate::OnMsgVelM(const interfaces::msg::MotorVels::SharedPtr _msgM)
	{
		RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "\nSending Motor Velocities");
		Platform1PluginPrivate::SetVelocityM1(_msgM->vel_m1);
		Platform1PluginPrivate::SetVelocityM2(_msgM->vel_m2);
		Platform1PluginPrivate::SetVelocityM3(_msgM->vel_m3);
		Platform1PluginPrivate::SetVelocityM4(_msgM->vel_m4);

		//time.sleep(0.0);
	}

	//Event update callback for publishers
	void Platform1PluginPrivate::OnUpdate(const gazebo::common::UpdateInfo & /*_info*/)
	{
		//Declaration of variables
		interfaces::msg::MotorVels velM;

		//Get velocity joints
		velM.vel_m1=Platform1PluginPrivate::jointM1->GetVelocity(0);
		velM.vel_m2=Platform1PluginPrivate::jointM2->GetVelocity(0);
		velM.vel_m3=Platform1PluginPrivate::jointM3->GetVelocity(0);
		velM.vel_m4=Platform1PluginPrivate::jointM4->GetVelocity(0);
		//Publish
		Platform1PluginPrivate::sim_vel_M_->publish(velM);
	}
	...
	//Reset
	void Platform1Plugin::Reset()
	{
	  impl_->ros_node_.reset();
	}


	GZ_REGISTER_MODEL_PLUGIN(Platform1Plugin)
}	//namespace gazebo_plugin_sim