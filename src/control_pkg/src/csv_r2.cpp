...
class Csv_R2 : public rclcpp::Node
{
	public:
		Csv_R2() : Node("csv_r2")
		{
           subs_data = this->create_subscription<interfaces::msg::DataControl>(
               "/robot_2/data_control", 1, std::bind(&Csv_R2::subs_data_callback,this,_1));
...
class Csv_Phase_R2 : public rclcpp::Node
{
	public:
		Csv_Phase_R2() : Node("csv_phase_r2")
		{
            subs_state = this->create_subscription<interfaces::msg::RobotState>(
               "/robot_2/state", 1, std::bind(&Csv_Phase_R2::subs_state_callback,this,_1));
...
int main(int argc, char * argv[])
{
	...
	auto node = std::make_shared<Csv_R2>();
    auto csv_phase_r2 = std::make_shared<Csv_Phase_R2>();


    rclcpp::executors::MultiThreadedExecutor executor;
    executor.add_node(node);
    executor.add_node(csv_phase_r2);
    executor.spin();
