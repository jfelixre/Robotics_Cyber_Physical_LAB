...
class Compute_Trajectory_R2 : public rclcpp::Node
{
    public:
        Compute_Trajectory_R2() : Node("compute_trajectory_r2")
        {
    ...
    private:
        ...
        void subs_callback(const interfaces::msg::Positions::SharedPtr pos_msg)
        {
            ...
            cv::fillConvexPoly(map_bin,vertices_R1, cv::Scalar(0));
            ...
            {
            case -1:
                Initial.position.x = R2_center_point.x;
                Initial.position.y = R2_center_point.y;
            ...
            case 1:
                goal.x = O2_point.x;// + (distance_objective * sin(O2_orientation_z));
                goal.y = O2_point.y + (distance_objective * cos(O2_orientation_z));
                Saved.position.x = O2_point.x;
                Saved.position.y = O2_point.y;
                //Saved.orientation.z = O2_orientation_z;
                break;
            ...
            request->src_x = R2_center_point.x;
            request->src_y = R2_center_point.y;
...
*/
            ...
            cv::circle(map_color,R2_point,1,cv::Scalar(0,0,255),1);
            cv::circle(map_color,R2_center_point,1,cv::Scalar(255,0,0),1);
            cv::circle(map_color,goal,1,cv::Scalar(255,0,0),1);

            //cv::namedWindow("Display Image", cv::WINDOW_NORMAL );
            //cv::imshow("Display Image", map);

            cv::namedWindow("MAP_R2", cv::WINDOW_NORMAL );
            cv::imshow("MAP_R2", map_color);
            cv::waitKey(1);
...
int main(int argc, char * argv[])
{
    ...
    auto node = std::make_shared<Compute_Trajectory_R2>();
    ...
    executor.spin();
    //rclcpp::spin(std::make_shared<Compute_Trajectory_R2>());
    rclcpp::shutdown();
    return 0;
}