...
class Control_PID_P1_node : public rclcpp::Node
{
	public:
		Control_PID_P1_node()
		: Node("control_pid_p1_node"), count_(0)
		{

			client = this->create_client<interfaces::srv::PlatformVel>("platform1_vel");

			subs_img_data = this->create_subscription<interfaces::msg::ImgData>(
				"/image_data", 1, std::bind(&Control_PID_P1_node::subs_callback,this, _1));

			timer_ = this->create_wall_timer(
      dt, std::bind(&Control_PID_P1_node::timer_callback, this));

      state_platform1_publisher = create_publisher<interfaces::msg::PlatState>("platform1_state",1);
      state_platform2_subscriber= create_subscription<interfaces::msg::PlatState>(
      	"/platform2_state", 1, std::bind(&Control_PID_P1_node::p2_subs,this,_1));
			
			data_platform_publisher = create_publisher<interfaces::msg::ControlData>("platform1_control_data",1);
	...
	private:
		...
		void p2_subs(const interfaces::msg::PlatState::SharedPtr msg) const
		{
			platform2_state=msg->platform_state;
		}

		void timer_callback()
		{
			if (platform1_state==-1)
			{
				RCLCPP_INFO(this->get_logger(), "Iniciando Nodo de Control de Plataforma 1");
				platform1_state = 0;
			}

			if (platform1_state==0)
			{
				RCLCPP_INFO(this->get_logger(), "Esperando Conexion de Plataforma 2");

				if (platform2_state==0 or platform2_state==1)
				{
					platform1_state=1;
					RCLCPP_INFO(this->get_logger(), "Iniciando Control de Plataforma 1");
					Xinit=Xrobot1;
					Yinit=Yrobot1;
					ANGinit=ANGrobot1;
				}
			...
			send_vel();
			 ...
			 msg_publ.platform_state=platform1_state;	
			 state_platform1_publisher->publish(msg_publ);
		...
		{
			Xgoal=Xobj1-(130*cos(ANGobj1));
			Ygoal=Yobj1-(130*sin(ANGobj1));
			ANGgoal=ANGobj1;
			//RCLCPP_INFO(this->get_logger(), "Entro a control_F1");
			PIDcicle();
			RCLCPP_INFO(this->get_logger(), "Error X R1 = %d", errorX);
		...
		{
			Xgoal=Xobj1-(200*cos(ANGobj1));
			Ygoal=Yobj1-(200*sin(ANGobj1));
			ANGgoal=ANGobj1;
			//RCLCPP_INFO(this->get_logger(), "Entro a control_F1");
			PIDcicle();
			RCLCPP_INFO(this->get_logger(), "Error X R1 = %d", errorX);
		...
		void send_vel()
		{
  		...
  		RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Velocities sent to Platform 1 server");
		...
		void save_data()
		{
			...
			ctrl_data_msg.fase = platform1_state;
		...
		size_t count_;
		rclcpp::Publisher<interfaces::msg::PlatState>::SharedPtr state_platform1_publisher;
		rclcpp::Subscription<interfaces::msg::PlatState>::SharedPtr state_platform2_subscriber;
...
int main(int argc, char * argv[])
{
  ...
  rclcpp::spin(std::make_shared<Control_PID_P1_node>());
