...
class Control_PID_R2_node : public rclcpp::Node
{
	public:
		Control_PID_R2_node()
		: Node("control_pid_r2_node"), count_(0)
		{

			client = this->create_client<interfaces::srv::RobotVel>("robot2_vel");

			subs_img_data = this->create_subscription<interfaces::msg::ImgData>(
				"/image_data", 1, std::bind(&Control_PID_R2_node::subs_callback,this, _1));

			timer_ = this->create_wall_timer(
      			dt, std::bind(&Control_PID_R2_node::timer_callback, this));

      		state_robot2_publisher = create_publisher<interfaces::msg::RobotState>("robot2_state",1);

     		state_robot1_subscriber= create_subscription<interfaces::msg::RobotState>(
      			"/robot1_state", 1, std::bind(&Control_PID_R2_node::r1_subs,this,_1));

      		data_robot_publisher = create_publisher<interfaces::msg::ControlData>("robot2_control_data",1);
			
	  		LmSw_subscriber= create_subscription<interfaces::msg::LimitSwitch>(
      			"/robot_2/limit_switch", 1, std::bind(&Control_PID_R2_node::LmSw_subs,this,_1));

			velM_subscriber= create_subscription<interfaces::msg::MotorVelsWArm>(
      			"/robot_2/sim_vel_Motors", 1, std::bind(&Control_PID_R2_node::velM_subs,this,_1));
	...
	private:
		...
		void timer_callback()
		{
			if (robot2_state==-1)
			{
				RCLCPP_INFO(this->get_logger(), "Iniciando Nodo de Control de Robot 2");
			...
			if (robot2_state==0)
			{
				...
				if (robot1_state==0 or robot1_state==1)
				{
				...
				if (abs(errorB1)<0.2 and abs(errorB2)<0.2 and abs(errorB3)<0.2)
				{
					arminit=1;
				}
			...
			if (robot2_state==8)
			{
				RCLCPP_INFO(this->get_logger(), "Control Robot 2 Terminado");
				velx=0;
				vely=0;
				velang=0;
				send_vel();
			...
			send_vel();
			 ...
			 msg_publ.robot_state=robot2_state;	
			 state_robot2_publisher->publish(msg_publ);
		...
		void control_F3()  //Retirarse de la base del objeto
		{
			...
			if (error_count>10 && robot1_state>=7){
				robot2_state=4;
				error_count=0;
			}
		...
		void send_vel()
		{
  		...
  		RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Velocities sent to Robot 2 server");
		...
		void save_data()
		{
			...
			ctrl_data_msg.fase = robot2_state;
...
{
	myfile.open ("Data_R2.csv");
  rclcpp::init(argc, argv);

  rclcpp::spin(std::make_shared<Control_PID_R2_node>());
