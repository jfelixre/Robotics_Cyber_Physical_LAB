...
class Control_Arm_Pid_R2 : public rclcpp::Node
{
	public:
		Control_Arm_Pid_R2() : Node("node_control_arm_pid_r2")
		{
            timer_pid_cb_group_ = this->create_callback_group(rclcpp::CallbackGroupType::Reentrant);

            timer_pid_ = this->create_wall_timer(
                50ms, std::bind(&Control_Arm_Pid_R2::timer_pid_callback, this), timer_pid_cb_group_);

            arm_position_subs= create_subscription<interfaces::msg::ArmJointsPositions>(
      			"/robot_2/set_arm_joints_position", 1, std::bind(&Control_Arm_Pid_R2::arm_pos_callback,this,_1));

            publisher_vel = this->create_publisher<interfaces::msg::MotorArmVels>("/robot_2/set_arm_joints_vel",10);
