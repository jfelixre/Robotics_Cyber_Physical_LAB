...
def generate_launch_description():
    ...
    convert1 = ExecuteProcess(
        cmd=[[
            'python3 ros2bag_to_csv.py bags/positions bags/positions_csv'
        ]],
        shell=True
    )

    convert2 = ExecuteProcess(
        cmd=[[
            'python3 ros2bag_to_csv.py bags/error_r1 bags/error_r1_csv'
        ]],
        shell=True
    )

    convert3 = ExecuteProcess(
        cmd=[[
            'python3 ros2bag_to_csv.py bags/error_r2 bags/error_r2_csv'
        ]],
        shell=True
    )

    convert4 = ExecuteProcess(
        cmd=[[
            'python3 ros2bag_to_csv.py bags/state bags/state_csv'
        ]],
        shell=True
    )
